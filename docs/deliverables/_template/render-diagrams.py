...
Convention: an image line immediately precedes its Mermaid source —

    ![Capability view](assets/capabilities.png)
    ```mermaid
    ...
    ```
...
Requires Node (uses `npx @mermaid-js/mermaid-cli`; the first run downloads a headless browser,
which takes a few minutes — run it once before a documentation session, not during).
First needed at step 0.6.
"""
import re
import subprocess
import sys
