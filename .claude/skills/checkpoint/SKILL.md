---
name: checkpoint
description: Run the active phase's checkpoint quiz — the user answers from memory before seeing the answer; on a pass, close the phase in ROADMAP.md and commit. Use at the end of a phase or when the user invokes /checkpoint.
---
...
1. **Find the active phase** in `ROADMAP.md`. All its steps must be ticked; if not, list what is
   missing and stop.
2. **Ask the checkpoint question** for that phase — the *Checkpoint question* column of
   `docs/PLAN.md` §5 (study phases also have it under their module in §4). Ask
   conversationally, one question at a time, and wait for the answer. Add one or two follow-ups
   drawn from the phase's learning docs ("Check yourself" sections) and, for a study phase,
   one question of the form "what did experiment X measure, and what number decided it?"
3. **Evaluate honestly.**
   ...
   - Wrong → give the correct answer with a pointer to the learning doc, the lab README or the
     code in this repo that demonstrates it. Recommend (don't force) re-reading before continuing.
